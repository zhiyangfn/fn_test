...
enum{
		...
		GPIO_PD0 = 0x300 | BIT(0), GPIO_GP6=GPIO_PD0,
		GPIO_PD1 = 0x300 | BIT(1), GPIO_GP7=GPIO_PD1,
		...
		GPIO_PE4 = 0x400 | BIT(4), GPIO_GP16=GPIO_PE4,
		GPIO_PE5 = 0x400 | BIT(5), GPIO_GP17=GPIO_PE5,
...
static inline u32 gpio_read_all(u8 *p){
	p[0] = REG_ADDR8(0x580);
	p[1] = REG_ADDR8(0x588);
	p[2] = REG_ADDR8(0x590);
	p[3] = REG_ADDR8(0x598);
...
{
	if (en) {
		BM_SET(reg_gpio_wakeup_irq, FLD_GPIO_WAKEUP_EN);
	}
	else {
		BM_CLR(reg_gpio_wakeup_irq, FLD_GPIO_WAKEUP_EN);
	}
}
