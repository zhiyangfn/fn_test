...
_attribute_ram_code_ void Debug_putchar(u8 byte)
{
	uart_put_char(byte);
}
...
//u8  USART_TX_BUF[256]; 			//发送缓冲,最大USART2_MAX_SEND_LEN字节
		...
		if(span != '%'){
			...
			if(span == 'c'){
				...
				puti(j,OCTAL_OUTPUT);
					...
					j = va_arg(arg_ptr,unsigned short int);//get hex value
					puti(j,HEX_OUTPUT);
			}else if(span == 0){
				break;
			}else{
