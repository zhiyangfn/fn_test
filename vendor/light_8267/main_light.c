...
u8 CT_VAL=50;
//////////////////////////////
u8 CURRENT_MODE=0;    // 1调色模式，  2 调光模式
...
void light_onoff_normal(u8 on){
    if(on){
        ...
        if(CURRENT_MODE==1)
        		...
        		if(CURRENT_MODE==2)
        		{
        			pwm_set_lum (PWMID_C1, get_pwm_cmp((100-CT_VAL)*2.5,led_lum), 0);//冷光
        ...
        if(CURRENT_MODE==1)
        		...
        		if(CURRENT_MODE==2)
        		{
        			pwm_set_lum (PWMID_C1, get_pwm_cmp(0,0), 0);//冷光
        			pwm_set_lum (PWMID_W, get_pwm_cmp(0,0), 0);
...
void light_lum_retrieve(void){
	...
	if(analog_read(rega_light_off)){
	    analog_write(rega_light_off, 0);
	    light_onoff(0);
	}else{
	    light_onoff(1);
	}
...
void rf_link_data_callback (u8 *p)
{
    ...
    if(op_cmd_len == op_type_1){
        ...
        if(vendor_id == VENDOR_ID){
            ...
            if(op == LGT_CMD_LIGHT_ONOFF){
               ...
               // light_adjust_RGB(led_val[0], led_val[1], led_val[2], led_lum);
                //处理手机上的亮度滑动条
                if(CURRENT_MODE==1)//调色
				{
					light_adjust_RGB(led_val[0], led_val[1], led_val[2], led_lum);//设置亮度
				}
				else
        	...
        	else if ((op == LGT_CMD_SET_RGB_VALUE) && (getLedStatus() == 1))
        	{
        		...
        		if(params[0] == 1){		        //R
        		   ...
        		   // light_adjust_RGB(led_val[0], led_val[1], led_val[2], led_lum);
                    ...
                    //light_adjust_RGB(led_val[0], led_val[1], led_val[2], led_lum);
                    CT_VAL = params[1];
                    if(CURRENT_MODE!=2)//调色
					{
                    	CURRENT_MODE=2;//模式修改为条光模式
						light_adjust_RGB(0,0,0, led_lum);//设置RGB为0
		...
		setWorkStatus(op,params,params_len);
    }
}
...
void main_loop(void)
{
	...
	if(clock_time_exceed(adc_tmp_check_time, 40*1000)){
        ...
        u32 v_ref = 3300;
        if (ADC_CHNM_REF_SRC_8267 == RV_AVDD){      // can not use "#if" because of enum
            v_ref = 3300;
        }else if(ADC_CHNM_REF_SRC_8267 == RV_1P224){
            v_ref = 1224;
        }else if(ADC_CHNM_REF_SRC_8267 == RV_1P428){
            v_ref = 1428;
        }
...
void  user_init(void)
{
...
#if 1	
	// for app ota   
	if(0 == is_ota_area_valid(ota_program_offset)){ // 8267 should use "ota_program_offset"
		erase_ota_data(ota_program_offset);
	}
#endif
	...
	if(p_vendor_user_init){
		...
		pwm_set (PWMID_W, PMW_MAX_TICK, PMW_MAX_TICK);
		pwm_set (PWMID_C1, PMW_MAX_TICK, PMW_MAX_TICK);
		...
		gpio_set_func (PWM_W, !AS_GPIO);
		gpio_set_func (PWM_C1,!AS_GPIO);
		pwm_set_cmp(PWMID_W,0);
	    pwm_set_cmp(PWMID_C1,0);
	}
...
#if(IBEACON_ENABLE)
	#if 0 // apple ibeacon
    if (*(u16 *) FLASH_ADR_BEACON_MAJOR != 0xFFFF){
    	u16 major_data = *(u16 *) FLASH_ADR_BEACON_MAJOR;
    	major_data = (major_data << 8) | ((major_data >> 8) & 0x00FF);
        ibeacon_customer_data.major = major_data;
    }
    if (*(u16 *) FLASH_ADR_BEACON_MINOR != 0xFFFF){
    	u16 minor_data = *(u16 *) FLASH_ADR_BEACON_MINOR;
    	minor_data = (minor_data << 8) | ((minor_data >> 8) & 0x00FF);
        ibeacon_customer_data.minor = minor_data;
    }
    if (*(u32 *) FLASH_ADR_BEACON_UUID != 0xFFFFFFFF){
		u8 customer_UUID[16] = {0};
    	flash_read_page(FLASH_ADR_BEACON_UUID, 16, customer_UUID);
		memcpy(ibeacon_customer_data.proximity_UUID, customer_UUID, 16);
    }
    if (*(u16 *) FLASH_ADR_BEACON_COMPANYID != 0xFFFF){
		ibeacon_customer_data.company_ID = *(u16 *) FLASH_ADR_BEACON_COMPANYID;
    }
    
    beacon_len = sizeof(ibeacon_adv_t);
    memcpy(iBeaconData, &ibeacon_customer_data, beacon_len);
    ...
    i2c_pin_initial(I2C_HW_SDA, I2C_HW_SCL);    // must after light_lum_retrieve() for 8267
    #if I2C_MASTER_MODE
    i2c_init(); // master mode
    #else
    i2c_slave_init(I2C_IRQ_EN); // slave mode
    #endif
#endif
